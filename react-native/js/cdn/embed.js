(function() {
  ...
  var ExpoSnack = {
    append: function(container, options) {
      ...
      options.sdkVersion =
        options.sdkVersion || container.dataset.snackSdkVersion;
      ...
      if (options.id) {
        iframe.src =
          'https://snack.expo.io/embedded/' + options.id + iframeQueryParams;
      } else {
        iframe.src =
          'https://snack.expo.io/embedded' +
          iframeQueryParams +
          '&waitForData=true';
      }
    ...
    initialize: function() {
      document
        .querySelectorAll(
          '[data-sketch-id], [data-snack-id], [data-snack-code]'
        )
